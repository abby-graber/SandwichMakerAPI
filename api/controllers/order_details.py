...
def update(db: Session, order_detail_id, order_detail):
    db_order_detail = db.query(models.Recipe).filter(models.Recipe.id == order_detail_id)
...
def delete(db: Session, order_detail_id):
    db_order_detail = db.query(models.Recipe).filter(models.OrderDetail.id == order_detail_id)
